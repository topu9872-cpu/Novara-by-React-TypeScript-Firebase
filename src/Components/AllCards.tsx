...
export const Card: React.FC<CardProps> = ({ product }) => {
  ...
  return (
    <div>
      <div
        ref={cardRef}
        className="product-card w-full mx-auto max-w-xs bg-white rounded-2xl p-3 shadow-xs border border-neutral-100 opacity-0 flex flex-col justify-between hover:shadow-md transition-shadow"
      >
        ...
        <div className="relative w-full aspect-4/3 sm:aspect-square rounded-xl overflow-hidden bg-neutral-100 flex items-center justify-center mb-3">
          <img
            src={product.image}
            alt={product.name}
            className="w-full h-full max-h-sm object-cover object-center transform hover:scale-105 transition-transform duration-500"
          />
        </div>
